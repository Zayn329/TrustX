...
import { sha256, mockSha256, generateRealTxHash, generateEcdsaSignature, hashEip712ProofPayload } from '../cryptoUtils';
...
describe('cryptoUtils (Real Web Crypto API & Cryptographic Utilities)', () => {
  ...
  it('computes EIP-712 typed data proof hash', async () => {
    const payload = {
      researcherDid: 'did:trust:0x71c8a1b2c3d4e5f6a7b8c9d0e1f2a3b4c5d6e7f8',
      bountyId: 'bounty-101',
      proofHash: '0xa8f391c49e8832a104b291c7784f1122aef902b54d6199321c882103410f11aa',
      nonce: 1
    };

    const hash = await hashEip712ProofPayload(payload);
    expect(hash).toMatch(/^0x[a-f0-9]{64}$/);
  });
});
