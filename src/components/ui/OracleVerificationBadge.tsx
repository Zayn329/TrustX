...
export const OracleVerificationBadge: React.FC<OracleVerificationBadgeProps> = ({ result }) => {
  return (
    <div className="tx-surface rounded-xl p-5 space-y-3">
      <div className="flex items-center justify-between border-b border-white/[0.07] pb-3">
        <div className="flex items-center gap-2">
          <Cpu className="w-5 h-5 text-[#D7FF3F]" />
          <span className="text-xs font-bold text-zinc-200">DEMO ORACLE / Automated Sandbox Runner</span>
        </div>
      ...
      <div className="bg-[#050505] p-3 rounded-md border border-white/[0.08] font-mono text-[11px] text-zinc-300 space-y-1">
        <div className="flex items-center gap-1.5 text-zinc-500 text-[10px] uppercase font-sans mb-1">
          <Terminal className="w-3.5 h-3.5 text-[#D7FF3F]" />
          <span>Chainlink Functions Execution Log</span>
        </div>
        {result.logs.map((log, idx) => (
          <div key={idx} className="text-zinc-400 leading-tight">
            {log}
          </div>
        ))}
      </div>
